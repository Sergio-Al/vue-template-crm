                        ...
                        <q-menu>
                          <q-list style="min-width: 100px" dense>
                            <q-item clickable v-close-popup>
                              <q-item-section
                                @click="
                                  openAlertDeletedRelationDocumentReservas(row)
                                "
...
const { getReservasDocuments } = useReservasStore();
...
const openDialogDocument = async () => {
  ...
  const listDocuments = await getReservasDocuments(props.id);

  cantDocs.value = listDocuments.length + 1;
};

const reloadDocument = async (idmodulo: string) => {
  documentRelation.value = await getReservasDocuments(idmodulo);
  cantDocs.value = documentRelation.value.length + 1;
};

const openAlertDeletedRelationDocumentReservas = (item: any) => {
...
onMounted(async () => {
  documentRelation.value = await getReservasDocuments(props.id);
});
