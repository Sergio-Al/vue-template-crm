                        ...
                        <q-menu>
                          <q-list style="min-width: 100px" dense>
                            <q-item clickable v-close-popup>
                              <q-item-section
                                @click="
                                  openAlertDeletedRelationDocumentLead(row)
                                "
                    ...
                    >
                      <q-menu>
                        <q-list style="min-width: 100px" dense>
                          <q-item clickable v-close-popup>
                            <q-item-section
                              @click="openAlertDeletedRelationDocumentLead(row)"
                              >Quitar</q-item-section
                            >
                          </q-item>
                          <!-- <q-item clickable v-close-popup>
...
const { getLeadsDocuments } = useLeadsStore();
...
const openAlertDeletedRelationDocumentLead = (item: any) => {
  show_add.value = true;
  deleteRelation.value.data.module_name = 'HANO_Lead';
  deleteRelation.value.data.module_id = props.id;
  deleteRelation.value.data.type_module = 'hano_lead_documents';
...
const openDialogDocument = async () => {
  ...
  const listDocuments = await getLeadsDocuments(props.id);
  cantDocs.value = listDocuments.length + 1;
};

const reloadDocument = async (idmodulo: string) => {
  documentRelation.value = await getLeadsDocuments(idmodulo);
  cantDocs.value = documentRelation.value.length + 1;
};

onMounted(async () => {
  documentRelation.value = await getLeadsDocuments(props.id);
});
...
const sendIdDocWeb = (item: any) => {
  extension.value = item.nombre.match(/\.([^.]+)$/)?.[1];
  if (
    extension.value === 'docx' ||
    extension.value === 'doc' ||
    extension.value === 'xlsx'
  ) {
    const downloadUrl = `${HANSACRM3_URL}/index.php?entryPoint=download&id=${item.id_view_documento}&type=Documents`;
    ...
    link.value = item.id_view_documento;
    viewdocument.value = `${HANSACRM3_URL}/upload/${item.id_view_documento}`;
  }
};
</script>
<style lang="sass">
