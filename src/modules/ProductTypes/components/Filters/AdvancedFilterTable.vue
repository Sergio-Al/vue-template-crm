...
import { base, FormInputs } from '../../utils/types';
...
//store
const tableStore = useProductTypesStore();
...
const selectAllInputs = () => {
  form_fields.value = [];
  if (todos.value) {
    form_fields.value = form.value.map((el: FormInputs) => {
...
const changeDate = (val: base) => {
  dataFilter.value.creation_date = val;
};

const clearFilter = () => {
  if (dateRangeRef.value?.length > 0) {
    dateRangeRef.value[0].resetData();
  }
  dataFilter.value = {
    account: '',
    name: '',
