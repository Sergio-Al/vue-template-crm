...
import { axios_NS_07, axios_LB_01, axios_LB_05, axios_PREFERENCES } from 'src/conections/axiosCRM';
import {
  // BasicInformation,
  Params,
  // base,
...
export const useProductTypeListService = () => {
...
export const createProduct = async (
  ...
  try {
    ...
    //const { data } = await axios_NS_07.post('/tipo-productos', dataProductType);
    const { data } = await axios_NS_07.post('/tipo-productos', dataProductType);
    return data;
  } catch (error) {
    throw error;
...
export const deleteMassiveData = async (data: any) => {
  try {
    data.items.forEach(async (element:any) => {
      await axios_NS_07.delete(`/tipo-productos/${element.id}`);
    });
  } catch (error) {
    throw error;
  }
