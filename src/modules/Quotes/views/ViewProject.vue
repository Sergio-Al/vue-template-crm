...
import { modeloProyect } from '../utils/types';
</script>
...
const filter = ref('');
const dataQuotes = ref<modeloProyect>({} as modeloProyect);
...
onMounted(async () => {
  ...
  const fields = ['iddivision_c', 'idamercado_c', 'idgrupocliente_c'];
...
const link =
  HANSACRM3_URL + '/index.php?module=Project&action=DetailView&record=';

const reloadProyect = async () => {
  documentRelation.value = await getAosQuotesGetInformationSubpanels(
    'project',
    props.id
  );
...
const functionNewProyectCRM3 = () => {
  linkCreated.value =
    HANSACRM3_URL +
    `/index.php?module=Project&action=EditView&&iddivision_c=${dataQuotes.value.iddivision_c}&idamercado_c=${dataQuotes.value.idamercado_c}&idgrupocliente_c=${dataQuotes.value.idgrupocliente_c}&return_module=AOS_Quotes&return_action=DetailView&return_id=${props.id}&relate_id=${props.id}&relate_to=aos_quotes_project`;
};
</script>
        ...
        <template v-if="documentRelation.length > 0">
          ...
          >
            <template v-slot:body="props">
              <q-tr>
                <q-td key="number_c" :props="props">
                  {{ props.row.number_c }}
                </q-td>
                <q-td key="name" :props="props">
                  <a
                    :href="link + props.row.idproyect"
                    target="_blank"
                ...
                <q-td key="estimated_start_date" :props="props">
                  {{ props.row.estimated_start_date }}
                </q-td>
                <q-td key="estimated_end_date" :props="props">
                  {{ props.row.estimated_end_date }}
                </q-td>
              </q-tr>
            </template>
          </q-table>
