...
import { SearchUser } from 'src/components/types/index';
...
const editor = ref(
  `<p></p>
      <p> </p>
      <p>Atento a sus comentarios<br/><br/><span style="font-size:medium;font-family:'arial black', 'avant garde';color:#000080;"><strong>HANSA CRM</strong></span><br/><span>` +
...
const formEmail = ref({
  ...
  altemailBody: '',
  from_email: ['Hansa CRM DEV &lt;crm_dev@hansa.com.bo&gt;'],
...
const enviarEmail = async () => {
  ...
  origen.value.push(
    usuarioLogueado.value + ' ' + '&lt;' + correoUsuarioLoagueado.value + '&gt;'
  );
  // formEmail.value.from_email = origen.value;

  formEmail.value.from_email = [origen.value[0], usuarioLogueado.value];
  ...
  formEmail.value.attachment.push(attachmentData.value);

  const dataSendEmail = {
    layout: {
      idquotes: idmodule.value,
      idamercado: idamercadomodule.value,
    },
    emailToSend: formEmail.value,
  };
  const response = await api.post(
    `${process.env.CRM4_LB_06}/send-mails`,
    dataSendEmail
  );
  if (response.data.status !== 500 && response.data.status !== 400) {
    ...
    Notification(
      'negative',
      'error',
      '<strong> Error! </strong> <br/> Algo salio mal al enviar el correo.'
    );
  }
...
const cambiaTemplate = async (index: number) => {
  ...
  let t1 = filterReg.value[index].body_html.replace(
    '$usuarioLogueado',
    usuarioLogueado.value
  );
  editor.value = t1;
...
const idmodule = ref('');
const idamercadomodule = ref('');
const nameadjunto = ref('');
const openDialog = (
  datagest: any,
  dataasunto?: any,
  database64?: any,
  ...
  if (iddivision != '03') {
    Notification(
      'warning',
      'exclamation',
      '<strong> Notificación! </strong> <br/> La división no cuenta con una plantilla para enviar por correo.'
    );
    return;
  }
  if (idamercado != '03_01' && idamercado != '03_06') {
    Notification(
      'warning',
      'exclamation',
      '<strong> Notificación! </strong> <br/> Seleccione un área de mercado. <br/> o el area de mercado no cuenta con una plantilla para enviar por correo.'
    );
    return;
  }

  idmodule.value = '' + idcotizacion;
  idamercadomodule.value = '' + idamercado;

  if (datagest.length > 0) {
    const hasEmail = datagest.some((item: any) => item.email !== '');
    if (!hasEmail) {
      Notification(
        'warning',
        'exclamation',
        '<strong> Notificación! </strong> <br/> No se cuenta con correos para enviar la cotización.'
      );
    }
  } else {
    Notification(
  ...
  datagest.forEach((element: any, index: any) => {
    if (!!element.email) {
      guestsSelected.value[index] = {
        module: element.type,
  ...
  attachmentData.value = {
    type: 'Notes',
    attributes: {
      created_by: idUsuarioLogueado.value,
      name: 'Envio de cotización ' + dataasunto,
      file_mime_type: 'application/pdf',
      file_url: '',
      filename: dataasunto.replace(/\s+/g, '_') + '_cotizacion.pdf',
      parent_name: '(no subject)',
      filecontents: 'database64',
    },
  };

  if (!!dataasunto) {
    asunto.value = 'Envio de cotización ' + dataasunto;
    nameadjunto.value = dataasunto.replace(/\s+/g, '_');
  }
};
...
<template>
  <dialog-component
    :size-dialog="
      seleccionar_plantilla == true || archivo == true
        ? 'dialog-lg'
        : 'dialog-md'
    "
    v-model="open"
  ...
  >
    ...
    <template #body>
      <div class="row q-col-gutter-xs">
          ...
          <div class="q-pa-md">
            <q-card class="col-12 q-pa-md" style="min-height: 82vh">
              <div class="row items-center q-mb-sm">
                <div
                  class="col-xl-1 col-lg-1 col-md-1 col-sm-1 col-xs-1 column items-center q-mb-sm"
                >
                  De:
                </div>
                <div
                  class="col-xl-11 col-lg-11 col-md-11 col-sm-12 col-xs-12 q-mb-sm"
                >
                  <q-avatar color="primary" text-color="white" size="sm">
                    {{ usuarioLogueado.charAt(0).toUpperCase() }}
                  </q-avatar>
                  <span class="q-ml-sm"
                    >{{ usuarioLogueado }} ({{ correoUsuarioLoagueado }})</span
                  >
                </div>
              </div>
              <q-separator />
                  ...
                  >
                    ...
                    <template v-slot:selected-item="scope">
                      ...
                      >
                        <span class="text-caption"
                          >{{
                            scope.opt.fullname || scope.opt.email_address
                          }}
                    ...
                    <template #option="slotProps">
                      <q-item clickable v-bind="slotProps.itemProps">
                        ...
                        <q-item-section>
                          <q-item-label>{{
                            slotProps.opt.fullname
                          }}</q-item-label>
                          <q-item-label caption lines="2">{{
                            formatModuleName(slotProps.opt.module)
                          }}</q-item-label>
                          <q-item-label caption lines="2">{{
                            slotProps.opt.email_address
                          }}</q-item-label>
                        </q-item-section>
                      </q-item>
                      <q-separator inset />
              ...
              <div class="row justify-end">
                <div>
                  <q-btn
                    flat
                    :class="cc == false ? 'text-grey' : 'text-blue'"
                    label="CC"
                    @click="cc = !cc"
                  />
                  <q-btn
                    flat
                    :class="cco == false ? 'text-grey' : 'text-blue'"
                    label="CCO"
                    @click="cco = !cco"
                  />
                </div>
              ...
              <q-card class="my-card" bordered>
                <q-card-section>
                  <div class="row">
                    ...
                    <div class="col-6 row flex items-center q-gutter-sm">
                      <q-item-label class="text-h7">Adjunto: </q-item-label>

                      <q-item-label class="text-h7"
                        >{{ nameadjunto }}_cotizacion.pdf
        ...
        <div
          ...
          v-if="seleccionar_plantilla == true || archivo == true"
        >
          <!-- {{formEmail}} -->
          <div class="q-pa-md">
                  ...
                  <div class="col-12">
                    <q-card class="my-card" flat>
                      ...
                      <q-scroll-area style="height: 76vh">
                        <q-list dense>
                          <template
                            v-for="(templatesEmails, index) in filterReg"
                            :key="index"
                          >
                            <q-item
                              class="q-my-sm"
                              clickable
                              :active="link == templatesEmails.name"
                              @click="cambiaTemplate(index)"
                            ...
                            >
                              ...
                              <q-item-section>
                                <q-item-label
                                  :class="
                                    link == templatesEmails.name
                                      ? 'text-white'
                                      : 'text-primary'
                                  "
                                  >{{
                                    templatesEmails.name.replace(/_/g, ' ')
                                  }}</q-item-label
                                >
                              </q-item-section>
                            </q-item>
    ...
    <template #footer>
      <q-btn
        ...
        class="q-mr-md"
        @click="enviarEmail"
        :disable="guestsSelected.length > 0 ? false : true"
        >Enviar</q-btn
      >
  ...
  />
  <GuestsDialog
    v-model="addGuestDialog2"
    v-model:selected="guestsSelectedCC"
    :tabs-filtered="['contacts', 'users']"
    detail="Destinatarios con copia"
  />
  <GuestsDialog
    v-model="addGuestDialog3"
    v-model:selected="guestsSelectedCO"
    :tabs-filtered="['contacts', 'users']"
