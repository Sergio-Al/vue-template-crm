  ...
  <q-card class="my-card" v-if="ActiveSqeleton" style="min-height: 80vh">
    <q-card-section>
      <div class="row col-12 justify-between">
        <div class="col-xl-5 col-lg-3 col-md-4 col-sm-12 col-xs-12 q-mb-sm">
          <q-input bottom-slots dense v-model="filter" placeholder="Buscar por nombre, estado y area mercado" >
            ...
            <template v-slot:append>
              <q-icon name="search" v-if="!filter" > </q-icon>
...
<script setup lang="ts">
  ...
  const { getContactsDeliveries } = ContactStore();
  ...
  onMounted(async () => {
    documentRelation.value = await getContactsDeliveries(props.id, userCRM.iddivision);
