...
import { useIdGenerator } from '../../composables/Core';
// import { AccountStore } from '../../store/AccountStore';
import { RowTableModel } from '../../utils/types/index';
import TableDialog from '../Dialogs/TableDialog.vue';
import { emailsTemplateColumns } from 'src/modules/Emails/data/datasource';
...
const makeLocalContact = (
  ...
  return contacts.map((val) => ({
    id: val.id || generateLocalId(),
    phone: val.number || val.phone,
    country_code: val.country_code,
    country: val.country,
    principal: val.principal === '1',
    whatsapp: val.whatsapp === '1',
    created: true,
  }));
};
const makeExternalContact = (
  localPhones: CountryContactsLocal[]
): CountryContact[] => {
  const externalContacts = localPhones.map((val) => {
    if (val.created === true)
      return {
        id: val.id,
        phone: val.phone,
...
const createContact = () => {
  ...
  resetLocalValue();
  emits('update:modelValue', makeExternalContact(accountPhonesLocal.value));
};
...
const createLocalPhones = (val: CountryContact[]) => {
  accountPhonesLocal.value = makeLocalContact(val);
};
    ...
    <q-form @submit="onSubmit" @reset="onReset" class="q-gutter-md">
      <div>
        <q-input
          ...
          unmasked-value
          label="Numero"
          :rules="[(val) => !!val || 'Se requiere', checkRepeatedPhone]"
        >
          <template #before>
            <q-select
