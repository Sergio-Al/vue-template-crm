    ...
    <q-card-section>
      <div class="row col-12 justify-between">
        ...
        <div class="col-xl-4 col-lg-6 col-md-7 col-sm-12 col-xs-12 q-mb-sm">
          <div class="row justify-end">
            <slot name="buttons">
              <q-btn class="desktop-only" color="primary" @click="$emit('openDialog')" label="Seleccionar"
                size="md" />
            </slot>
          </div>
        </div>
      ...
      <div class="row">
        <div class="col-xs-12 col-md-3 q-pa-md" >
          <div v-if="filterdocRelation.length>0 && !$q.screen.xs " col-3 q-pa-md column items-end >
            <q-scroll-area
      style="height: 70vh; ">
               <q-list class="q-none" >
              <template v-for="row,index in filterdocRelation" :key="index">
                <q-item class="q-my-sm" clickable @click="sendIdDocWeb(row.id_view_documento)"
                  active-class="my-menu-link" :active="link === row.id_view_documento" >
                  ...
                  <q-item-section>
                    ...
                    <q-item-label caption lines="1" class="text-black">{{ row.categoria }}</q-item-label>
                    <q-item-label caption lines="1">Publicación: {{ row.fecha_publicacion }}</q-item-label>
                    <q-item-label caption lines="1">Vencimiento: {{ row.fecha_vencimiento }}</q-item-label>
                  </q-item-section>
                  <!-- <q-item-section side>
            ...
            <q-list>
              <template v-for="(row, index) in filterdocRelation" :key="index">
                ...
                >
                  ...
                  <q-item-section>
                   <q-item-label><a :href="link2+row.id_view_documento" >{{ row.nombre }}</a></q-item-label>
                   <q-item-label caption lines="1" class="text-black">{{ row.categoria }}</q-item-label>
                    <q-item-label caption lines="1">Publicación: {{ row.fecha_publicacion }}</q-item-label>
                    <q-item-label caption lines="1">Vencimiento: {{ row.fecha_vencimiento }}</q-item-label>
                  </q-item-section>                  
        ...
        <div class="col-9 q-pa-md column items-end" v-if="!$q.screen.xs">     
          ...
          <template v-else>
            <iframe :src="viewdocument" style="height: 70vh; width:100%;" frameborder="0">
            </iframe>
          </template>
        </div>
        <template>
          <q-dialog v-model="alert">
            <q-card>
              <iframe :src="viewdocument" style="height: 80vh; width:100%;" frameborder="0">
              </iframe>
            </q-card>
          </q-dialog>
        </template>
...
<script setup lang="ts">
  ...
  const { getContactsDocuments } = ContactStore();
  ...
  onMounted(async () => {
    documentRelation.value = await getContactsDocuments(props.id);
  })
  const filterdocRelation = computed(() => {
    return documentRelation.value.filter(
      (objeto) => objeto.nombre.toLowerCase().indexOf(filter.value.toLowerCase()) > -1
    );
  })
