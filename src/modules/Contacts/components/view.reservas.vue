      ...
      <div class="row col-12 justify-between">
        <div class="col-xl-2 col-lg-3 col-md-4 col-sm-12 col-xs-12 q-mb-sm">
          ...
          <template v-slot:hint>
             <span class="text-primary">{{filterdocRelation.length==1? filterdocRelation.length+' Reserva encontrada':filterdocRelation.length+' Reservas encontradas'}} </span> 
            </template>
      ...
      <div class="col-xs-12 col-md-3 q-pa-md">
        <template v-if="filterdocRelation.length > 0">
          <q-table
            ...
            hide-bottom
          >
            <!-- :card-class="$q.screen.xs ? 'bg-primary text-white':''" -->
              ...
              <q-tr :props="props">
                <q-td auto-width>
                  <q-btn
                    size="sm"
                    round
                    :color="props.row.estado == 'Confirmado' ? 'green' : props.row.estado == 'Rechazado'? 'red' : props.row.estado == 'Cerrado'? 'teal' : props.row.estado == 'Bloqueado'? 'orange'
                    : 'grey'"
                    :icon="
                      props.row.estado == 'Confirmado' ? 'check' : props.row.estado == 'Rechazado'? 'close' : props.row.estado == 'Cerrado'? 'done_all' : props.row.estado == 'Bloqueado'? 'block'
                    : 'edit_off'"
                  >
                  </q-btn>
                </q-td>
                ...
                >
                  <q-chip
                    :color="
                     props.row.estado == 'Confirmado' ? 'teal' : props.row.estado == 'Rechazado'? 'red' : props.row.estado == 'Cerrado'? 'teal' : props.row.estado == 'Bloqueado'? 'orange'
                    : 'grey'"
                    size="xs"
                    text-color="white"
                  >
...
const { getContactsReservations } = ContactStore();
...
onMounted(async () => {
  documentRelation.value = await getContactsReservations(props.id, userCRM.iddivision);
