    ...
    <q-card flat>
      ...
      <q-card-section >
        <q-input autogrow outlined bottom-slots v-model="text" placeholder="Escriba su comentario" :dense="dense"
          v-if="idContacts !== '0'" color="primary">
...
<script lang="ts" setup>
 const user = userStore();
  ...
  onMounted(async () => {
    commentslist.value = await getAccountComments(idContacts.value);
    commentslistAux.value = await getAccountComments(idContacts.value);
  });
  const addComments = async (val: string) => {
    loadComment.value=true;
    commentSend.value.comments.description = val;
    commentSend.value.comments.idmodule = idContacts.value;
  ...
  const deleteComments = async (id:string) => {
      ...
      commentDeleteSend.value.comments.idmodule = idContacts.value;
      const response = await api.post(
