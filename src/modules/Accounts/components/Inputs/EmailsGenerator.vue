...
const deletedEmails = ref([] as EmailAccountLocalModel[]);
...
const makeExternalEmail = (
  value: EmailAccountLocalModel
): EmailAccountExternalModel => {
  if (value.isLocal)
    return {
      id: '',
...
const makeExternalEmails = (
  emails: EmailAccountLocalModel[]
): EmailAccountExternalModel[] => {
  return emails.map((value) => makeExternalEmail(value));
};
...
const updateToParent = (emails: EmailAccountLocalModel[]) => {
  ...
  emits('update:modelValue', makeExternalEmails(emails));
};
...
const deleteEmail = (id: string | undefined) => {
  if (id) {
    ...
    const targetDeleted = accountEmails.value.find((val) => val.id === id);
    if (targetDeleted) deletedEmails.value.push(targetDeleted);
