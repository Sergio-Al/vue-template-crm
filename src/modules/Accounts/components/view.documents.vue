...
const openDialogDocument = async () => {
  ...
  const listDocuments = await getAccountsDocuments(props.idAccount);
...
const deleteDocument = async () => {
  ...
  await reloadDocument(props.idAccount);
  alertDelet.value = false;
  relacarga.value = false;
};

const reloadDocument = async (idmodulo: string) => {
  documentRelation.value = await getAccountsDocuments(idmodulo);
  cantDocs.value = documentRelation.value.length + 1;
};

onMounted(async () => {
  documentRelation.value = await getAccountsDocuments(props.idAccount);
});
