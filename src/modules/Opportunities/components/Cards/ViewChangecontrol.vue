...
const { gethistorychanges, getHistoryInitial } = useOpportunitiesStore();
...
onMounted(async () => {
  historyInitiallist.value = await getHistoryInitial(id.value, 'Opportunities');
  historyChangeslist.value = await gethistorychanges(id.value);
});
</script>
<style scoped>
#mapid {
