...
const { getListEntregas } = useOpportunitiesStore();
...
onMounted(async () => {
  documentRelation.value = await getListEntregas(props.id);
...
const link =
  HANSACRM3_URL + '/index.php?module=HANE_Entregas&action=DetailView&record=';
</script>
<template>
              ...
              <q-tr :props="props" >
                ...
                <q-td key="nombre" :props="props">
                  ...
                  <q-item-label lines="5" >
                    <a class="text-bold cursor-pointer flex items-center text-primary" @click="() => openDialog(props.row.id)"
         >{{props.row.nombre}}</a>
                  </q-item-label>
                  <!-- </a> -->
                </q-td>
                ...
                >
                  <q-chip
                    :color="
                            props.row.estado == 'Entregado' ? 'green' :
                            props.row.estado == 'Entregado y Verificado' ? 'teal' :
                            props.row.estado == 'En progreso' ? 'orange' :
                            props.row.estado == 'Pendiente' ? 'blue' :
                            props.row.estado == 'Cancelado' ? 'red' :
                            'grey'
                    "
                    size="xs"
                    text-color="white"
  ...
  <DeliveriesDialog ref="reserveDialogRef"  />
</template>
