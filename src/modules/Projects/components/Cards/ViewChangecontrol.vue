<template>
  ...
  <div v-else>
    <q-card flat>
      <q-card-section
        style="max-height: 70vh; padding-top: 0px"
        class="scroll"
        v-if="historyChangeslist.length > 0"
      >
            ...
            <q-item style="padding: 0px">
              ...
              <q-item-section>
                <q-item-label caption
                  >Creado por:
                  <span class="text-primary">{{ reg.creado_por }}</span>
                </q-item-label>
          ...
          <q-item-section>
            <q-item style="padding: 0px">
              ...
              <q-item-section>
                <q-item-label caption
                  >Modificado por:
                  <span class="text-primary">{{ reg.modificado_por }}</span>
                </q-item-label>
                <q-item-label caption lines="1">{{
        ...
        <q-timeline color="secondary">
          <q-timeline-entry
            v-for="(reg, index) in historyChangeslist"
            :key="index"
            icon="person"
          ...
          >
            <template v-slot:title>
              ...
              <span class="text-grey-7 text-capitalize text-caption">
                | {{ reg.creado_por + ' ' }}</span
              >
              ...
              <q-list bordered separator>
                <q-item clickable>
                  <q-item-section>
                    <q-item-label caption>
                      Campo modificado:
                      <span class="text-primary">{{ reg.campo }} </span>
                    </q-item-label>
                    <q-item-label caption>
                      <q-icon
                        name="check"
...
//const { getContactHistoryChanges } = ContactStore();
const { getHistoryInitial } = useProspectStore();
const props = defineProps<{
  idContacts: string;
}>();
const idContacts = ref(props.idContacts);
const historyInitiallist = ref([] as { [key: string]: string }[]);
const historyChangeslist = ref([] as { [key: string]: string }[]);
onMounted(async () => {
  historyInitiallist.value = await getHistoryInitial(
    idContacts.value,
    'Project'
  );
  //historyChangeslist.value = await getContactHistoryChanges(idContacts.value);
});
</script>
<!-- <style scoped>
#mapid {
