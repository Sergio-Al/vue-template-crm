...
const { getLeadsQuotes } = useLeadsStore();
...
const getHanoLeadInformation = ref();
...
onMounted(async () => {
  getHanoLeadInformation.value = await quotesStore.getModuleTypeIDPro(
    'HANO_Lead',
    props.id
  );
  console.log(getHanoLeadInformation.value);
  if (userCRM.iddivision == '') {
    userCRM.iddivision = 'todas';
  }
  documentRelation.value = await getLeadsQuotes(props.id, userCRM.iddivision);
...
const createNewQuotes = async (idprospect: string) => {
  dataOfSubPanelforQuotes.value = {
    ...
    datInfocarQuotes: {
      name: getHanoLeadInformation.value.attributes.name,
      iddivision: '',
      idamercado: '',
      region: '',
      idgrupocliente: '',
    ...
    prospectid: '',
    prospectname: '',
    leadid: getHanoLeadInformation.value.id,
    leadname: getHanoLeadInformation.value.attributes.name,
    accountId: '',
    accountName: '',
    contactId: '',
    contactName: '',
...
<template>
  ...
  <AlertComponent
    ...
    icon-color="warning"
    icon-size="70px"
    title="La cotizacion ya cuenta con un Lead"
    btnColor="primary"
    btnText="Si, relacionar"
    @confirm="saveRelationQuotesv2"
    @denegate="alert = false"
  >
    <template #body>
      <span class="text-center text-primary col-12">
        ...
        ya tiene con una Lead asociado
      </span>
      <span class="text-center col-12"
        >¿ Desea relacionar la cotización ?
      </span>
      <span class="text-center col-12"
        >Esto quitara la relación con el otro Lead
      </span>
    </template>
  </AlertComponent>
</template>
