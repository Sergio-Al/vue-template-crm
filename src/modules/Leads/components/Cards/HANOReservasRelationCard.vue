...
const getHanoReservasDetail = async (id: string, assign = false) => {
  if (assign) isLoading.value = false;
  const response = await getRecordModuleInfo('HANQ_Reservas', id, {
    allData: false,
...
const assignProspect = async () => {
  ...
  if (prospectFiltered.value.id) {
    ...
    getHanoReservasDetail(prospectFiltered.value.id, true);
  }
};
...
const { state, isLoading } = useAsyncState(async () => {
  if (props.id) {
    const response = await getHanoReservasDetail(props.id);
    return response;
  }
  return defaultData;
}, defaultData);
