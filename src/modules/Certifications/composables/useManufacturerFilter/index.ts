...
// import { searchAccounts } from '../../services/AccountRelatedService';
import Notification from 'src/composables/notify';

export const useContacts = (account_id?: string) => {
  ...
  const formInputs = ref([
    ...
    // },
    { label: 'Cuenta', field: 'account', visible: true },
    {
      input: 'q-input',
      label: 'Nombre(s)',
      clearable: true,
  ...
  const existChanges = (params: any) => {
    return Object.values(params).filter(
      (el: string | string[]) => el !== '' && el.length > 0
    ).length;
  };

  const submitFilter = async (params: any, account_id: string) => {
    // console.log(params);
