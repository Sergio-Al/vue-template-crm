          ...
          <div class="row justify-end q-gutter-sm">
            <slot name="buttons">
              <q-btn :class="!$q.screen.xs ? 'q-ms-md':'full-width'" color="primary" target="_blank" :href="linkCreated"
                label="Nuevo" size="md" />
              <q-btn :class="!$q.screen.xs ? 'q-ms-md':'full-width'" color="primary" @click="$emit('openDialog')" label="Seleccionar"
                size="md" />
...
<script setup lang="ts">
  ...
  const { getProspectsQuotes } = useProspectStore();
  ...
  onMounted(async () => {
    ...
    documentRelation.value = await getProspectsQuotes(props.id, userCRM.iddivision);
  ...
  }[] = [
  {
    ...
    const linkCreated = HANSACRM3_URL + '/index.php?module=AOS_Quotes&action=EditView&account_id='+props.id+'&account_name='+props.nameAccount;
</script>
