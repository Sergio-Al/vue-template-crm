...
<template>
  ...
  <q-card flat class="bg-grey-light-1 q-mb-sm" v-else style="width: 60vh;">
    <q-list separator>
      <template v-if="changing">
        <q-card flat class="bg-transparent q-pa-sm">
          <q-card-section class="q-pa-none">
              ...
              >
                <template #append>
                  <q-icon color="primary" name="search" v-if="search == ''" />
                  <q-icon
                    name="clear"
                    color="primary"
      ...
      <q-item class="item-list" v-if="userSelected != null">
        <q-item-section avatar>
          <q-avatar size="60px" class="shadow-1 bg-grey-3">
            <img :src="`${HANSACRM3_URL}${userSelected.avatar}`" alt="" />
          </q-avatar>
        </q-item-section>
        <q-item-section class="">
          <q-item-label>
            <div class="flex items-center text-primary justify-between">
              {{ userSelected.user_name }}
              <q-chip class="text-white" color="primary" size="sm">
                <q-icon
                  name="badge"
                  color="white"
                  class="q-mr-xs"
        ...
        <q-item-section side class="buttons-assigned">
          <q-btn v-if="!changing" dense round @click="changeUser" color="primary">
            <q-icon name="edit" color="white" size="xs" />
