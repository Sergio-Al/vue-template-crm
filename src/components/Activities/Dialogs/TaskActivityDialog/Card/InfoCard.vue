...
<script lang="ts" setup>
  ...
  const assignedUserId = ref(
    localValue.value.assigned_user_id 
  );
  ...
  const info = async () => {
    ...
    fechas.value.attributes.date_start= (moment(localValue.value.date_start).format('YYYY-MM-DD'));
    fechas.value.attributes.time_start= (moment(localValue.value.date_start).format('HH:mm'));
    fechas.value.attributes.date_end= (moment(localValue.value.date_due).format('YYYY-MM-DD'))
    fechas.value.attributes.time_end= (moment(localValue.value.date_due).format('HH:mm')) ; 
  };
  ...
  const deleteTasks = async () => {
    ...
    const response = await axios_LB_04.delete(`/tasks-delete/${props.idActivity}/${userCRM.id}`);
    Loading.hide();
    emits('formSaved');
    open.value = false;
  };
  ...
  const saveTask = async () => {
    ...
    let new_date_end = moment(fechas.value.attributes.date_end, 'YYYY-MM-DD').format('YYYY-MM-DD');
  ...
  const isEditing2 = computed(() => baseCardRef.value?.isEditing === 'edit');
...
<template>
  <dialog-component size-dialog="dialog-lg" v-model="open" :footerDisabled="!isEditing2" :headerDisabled="false"
    :iconDialog="''" :persistent="false" @show="info">
    <template #header>
        ...
        <q-toolbar class="bg-primary q-pa-lg">
          <q-list>
            <q-item>
              ...
              <q-item-section>
                <q-item-label class="text-grey-5 text-caption" lines="1">Tarea
      ...
      <div class="row q-pa-md q-col-gutter-sm">
        <div class="col-12 col-sm-6 q-gutter-y-sm">
          <div class="row" style="height: 75vh;">
            <view-card-component style="width: 100%" :controls="!!idActivity"
              :initial-status="!!idActivity ? 'read' : 'edit'" icon-name="task" ref="baseCardRef"
              title="Información de la tarea" @cancel-change="restoreValues">
              <template #read>
                <div class="row q-col-gutter-sm q-ma-xs">
                  <q-select class="col-12 col-sm-12 q-mb-sm" v-model="inputData.tipotarea_c" hide-dropdown-icon
                    :options="optionTypeTask" stack-label dense label="Tipo de tarea" option-value="value" option-label="label"
                    emit-value map-options color="secondary" readonly>
                  </q-select>
                  <q-input class="col-12 col-sm-12 q-mb-sm" readonly dense v-model="inputData.name" type="text"
                    label="Asunto" />
                  <q-select class="col-12 col-sm-12 q-mb-sm" v-model="inputData.status" hide-dropdown-icon
                    :options="optionsEstado" stack-label dense label="Estado" option-value="value" option-label="label"
                    emit-value map-options color="secondary" readonly>
                  </q-select>
                  <q-input class="col-12 col-sm-12 q-mb-sm" autogrow v-model="inputData.description" type="textarea"
                    label="Descripcion" readonly>
                  </q-input>            
                  <div class="row q-col-gutter-sm col-12">
                    <div class="col-6 col-xs-6">
                      <q-input v-model="fechas.attributes.date_start" label="Fecha de Inicio" type="date" readonly dense
                        color="primary">
                        <template #before>
                          <q-icon name="event" />
                        </template>
                    ...
                    <div class="col-6 col-xs-6">
                      <q-select color="grey-3" readonly dense v-model="fechas.attributes.time_start" :options="horas"
                        label="Hora inicio">
                    ...
                    <div class="col-6 col-xs-6">
                      <q-input v-model="fechas.attributes.date_end" label="Fecha de Fin" type="date" readonly dense
                        color="primary">
                        <template #before>
                          <q-icon name="event" />
                        </template>
                    ...
                    <div class="col-6 col-xs-6">
                      <q-select color="grey-3" readonly dense v-model="fechas.attributes.time_end" :options="horas"
                        label="Hora Fin">
                        <template v-slot:append>
                          <q-icon name="schedule" />
                        </template>
              ...
              <template #edit>
                <div class="row q-col-gutter-sm q-ma-xs">
                  <q-select class="col-12 col-sm-12 q-mb-sm" v-model="inputData.tipotarea_c" :options="optionTypeTask"
                  ...
                  <q-input ref="myForm1" class="col-12 col-sm-12 q-mb-sm" borderless dense v-model="inputData.name"
                    type="text" label="Asunto*" outlined>
                    <template #before>
                      <q-icon name="title" />
                    </template>
                  </q-input>
                  <q-select class="col-12 col-sm-12 q-mb-sm" v-model="inputData.status" :options="optionsEstado"
                    stack-label outlined dense label="Estado" option-value="value" option-label="label" emit-value
                    map-options color="secondary">
                    <template #before>
                      <q-icon name="list" />
                    </template>
                  </q-select>
                  <q-input class="col-12 col-sm-12 q-mb-sm" v-model="inputData.description" type="textarea"
                    label="Descripcion" outlined autogrow>
                    <template #before>
                      <q-icon name="list" />
                    </template>
                  </q-input>
                  <div class="row q-col-gutter-sm col-12">
                    <div class="col-6 col-xs-6">
                      <q-input v-model="fechas.attributes.date_start" label="Fecha de Inicio" type="date" outlined dense
                        color="primary">
                        <template #before>
                          <q-icon name="event" />
                        </template>
                    ...
                    <div class="col-6 col-xs-6">
                      <q-input v-model="fechas.attributes.date_end" label="Fecha de Fin" type="date" outlined dense
                        color="primary">
                        <template #before>
                          <q-icon name="event" />
                        </template>
        ...
        <div class="col-12 col-sm-6 q-gutter-y-sm">
          <div class="row">
            <q-card style="width: 100%">
              <AssignedSingleUserTask ref="assignedSingleUserRef" :module="'tasks'" :module-id="idActivity"
                :withList="true" @changeUser="onChangeUserAssigned" @new-assigned-id="updateAssignedUserId" />
            </q-card>
          </div>
          <div class="row">
            <q-card style="width: 100%">
              ...
              <q-tab-panels v-model="tab" animated style="min-height: fit-content">
                <q-tab-panel name="comentarios" style="min-height: 59vh" class="q-py-sm">
                  <CommentsList :id="idActivity" :descCRM3="''" :modulo="'Tasks'"></CommentsList>
                </q-tab-panel>
              </q-tab-panels>
            </q-card>
          </div>
    ...
    <template #footer v-if="isEditing2">
