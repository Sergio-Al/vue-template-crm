...
const usePais = () => {
  ...
  const listPais = ref<PaisModel[]>([]);
  const listRegion = ref<RegionModel[]>([]);
  try {
...
const useResponsiblesRoles = () => {
  const listResponsiblesRoles = ref([]);
  try {
    const getlistResponsiblesRoles = async () => {
      const { data } = await axios_LB_05.get(
        '/lang-es-responsible-roles-lists'
      );
...
const useProjectStatus = () => {
  const listProjectStatus = ref([]);
  try {
    const getlistProjectStatus = async () => {
      const { data } = await axios_LB_05.get('/lang-es-project-status-lists');
...
const useProjectMetricUnits = () => {
  const listProjectMetricUnits = ref([]);
  try {
    const getlistProjectMetricUnits = async () => {
      const { data } = await axios_LB_05.get(
        '/lang-es-project-metrics-units-dom-lists'
      );
